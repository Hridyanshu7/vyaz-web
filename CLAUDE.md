...
- **Session history + rating:** every Talk/Gist session (all 3 providers) is durably recorded in **`voice_sessions`** (transcript + metadata) and ends with a mandatory 1-5★ + optional-text rating shown inline in the modal. See DECISIONS (migrations `008`/`009`).
